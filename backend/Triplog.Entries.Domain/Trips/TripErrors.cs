...
{
    public static readonly Error TitleEmpty = Error.Validation(
        "Trip.TitleEmpty", "Trip title cannot be empty.");

    public static Error TitleTooLong(int actualLength) => Error.Validation(
        "Trip.TitleTooLong", $"Trip title length {actualLength} exceeds the maximum of {TripTitle.MaxLength}.");
    ...
    public static readonly Error InvalidStatusTransition = Error.Validation(
        "Trip.InvalidStatusTransition", "The requested status transition is not allowed from the current status.");

    public static readonly Error AlreadyArchived = Error.Validation(
        "Trip.AlreadyArchived", "Trip is already archived.");
}
